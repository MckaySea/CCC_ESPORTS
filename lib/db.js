...
// CRITICAL NODE.JS FIX: This is a direct global workaround for the persistent
// 'self-signed certificate in certificate chain' error in deployment environments.
// This forces Node.js to ignore the certificate chain validation globally for this process.
...
// CRITICAL ADJUSTMENT: Ensure the connection string forces SSL mode to require.
// This is often needed in serverless environments like Vercel to properly initiate the SSL handshake
// before the rejectUnauthorized: false setting takes effect.
if (!CONNECTION_STRING.includes("sslmode")) {
  CONNECTION_STRING += CONNECTION_STRING.includes("?")
    ? "&sslmode=require"
    : "?sslmode=require";
}
